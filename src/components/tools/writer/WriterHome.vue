<template>
  <div id="ToolHome" v-if="!$root.$data.session.writer.selectedToolId">
    <div class="wavemaker_info_box">
      <table style="width: 100%">
        <tr>
          <td><h1>Manuscripts</h1></td>
      ...
      <table style="width: 100%">
        <tr v-for="(item, index) in mylist" :key="index">
          ...
          <td style="width: 100px">
            <button
              @click="$root.$data.session.writer.selectedToolId = item.uuid"
              class="formButton"
            >
              <svg style="width: 24px; height: 24px" viewBox="0 0 24 24">
                <path
  ...
  <WriterTool v-if="$root.$data.session.writer.selectedToolId" />
</template>
...
export default {
  ...
  computed: {
    mylist() {
      let list = {};
      Object.keys(this.$root.$data.shadowDB.Writer).forEach((element) => {
        if (
          this.$root.$data.shadowDB.Writer[element].projectID ===
          this.$root.$data.session.selectedProject
        ) {
          list[this.$root.$data.shadowDB.Writer[element].uuid] =
            this.$root.$data.shadowDB.Writer[element];
        }
      });
      return list;
    },
