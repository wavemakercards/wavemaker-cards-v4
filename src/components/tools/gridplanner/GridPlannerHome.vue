<template>
  <div id="ToolHome" v-if="!$root.$data.session.gridplanner.currentboard">
    <h1>GridPlanner</h1>
    ...
    <div class="boards" v-if="!showAddForm">
      <button
        ...
        v-for="(b, i) in mylist"
        :key="i"
        @click="$root.$data.session.gridplanner.currentboard = b.uuid"
      >
        <h1>{{ b.title }}</h1>
        <p>{{ b.description }}</p>
      </button>
  ...
  <GridPlanner  v-if="$root.$data.session.gridplanner.currentboard" />
</template>
...
export default {
  ...
  data() {
    ...
    mylist(){
      ...
      Object.keys(this.$root.$data.shadowDB.Gridplanner).forEach(element => {
        if(this.$root.$data.shadowDB.Gridplanner[element].projectID === this.$root.$data.session.selectedProject){
          list[this.$root.$data.shadowDB.Gridplanner[element].uuid] = this.$root.$data.shadowDB.Gridplanner[element]
        }
      });
      return list
    }
  ...
  methods: {
    AddBoard() {
      ...
      this.$root.$data.shadowDB.Gridplanner[obj.uuid] = obj;
  ...
  beforeMount() {
    ...
    if (!this.$root.$data.session.gridplanner) {
      this.$root.$data.session.gridplanner = {};
    }
  },
};
</script>
