<template>
  <button
    @click="$root.GoogleDriveSignIn()"
    v-if="!$root.$data.GoogleDriveApi.loggedin"
    class="formButton GoogleDrive"
  >
...
<h3>Save the current project to google drive</h3>
<h4>{{this.$root.$data.shadowDB.Settings[Object.keys(this.$root.$data.shadowDB.Settings)[0]].settings.ProjectName}}</h4>
<button @click="$root.GoogleDriveWriteFile()" class="formButton">Save File</button>
    </div>
     ...
     <h3>Your Files</h3>
<div >
<button @click="$root.$data.GoogleDriveApi.files=[];$root.GoogleDriveListFiles();" class="formButton filebutton" style="width:auto;">
...
<div v-if="$root.$data.GoogleDriveApi.files.length">
    <div v-for="(f, i) in $root.$data.GoogleDriveApi.files" :key="i">
      <button @click="openFile(f)" class="formButton filebutton file">
        <svg style="width: 24px; height: 24px" viewBox="0 0 24 24">
          ...
          <path
            id="path3731"
            d="m 11.084747,9.8305089 h 7.186439 V 17.01695 h -7.186439 z"
...
<hr/>
  <button
        @click="$root.GoogleDriveSignOut"
        class="formButton filebutton" style="width: auto; display:inline-block"
      >
        <svg style="width: 24px; height: 24px" viewBox="0 0 24 24">
          <path
            fill="#fff"
    ...
    <button class="formButton filebutton logout" @click="$root.$data.popup.name=null">
      <svg style="width:24px;height:24px" viewBox="0 0 24 24">
...
Close
    ...
    <!-- 
    <div>
  <textarea v-model="$root.GoogleDriveApi.CURRENT_FILE_CONTENTS"></textarea>
    <button @click="$root.GoogleDriveWriteFile()">Save File</button>
    </div>
    <div>
    <input v-model="$root.$data.GoogleDriveApi.CURRENT_FILE_NAME" />
    <button @click="$root.GoogleDriveWriteFile()">Create New File</button>
    </div>
    -->
</div>
...
export default {
  ...
  methods: {
    ...
    openFile(f) {
      this.$root.$data.GoogleDriveApi.CURRENT_FILE_OBJ = f;
      this.$root.$data.GoogleDriveApi.CURRENT_FILE_NAME = f.name;
