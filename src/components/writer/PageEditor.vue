...
<svg version="1.1" width="24" height="24" viewBox="0 0 24 24"><path d="M14.6,16.6L19.2,12L14.6,7.4L16,6L22,12L16,18L14.6,16.6M9.4,16.6L4.8,12L9.4,7.4L8,6L2,12L8,18L9.4,16.6Z" /></svg>
    ...
    <div class="wordcountdisplay">{{ wordcount }} of {{ this.$root.calcFullWordCount }} </div>
...
export default {
  ...
  mounted() {
    this.editor = new Editor({
      ...
      // content: this.$root.session.writer.file.content,
      onUpdate: () => {
        // HTML
        ...
        // let mykey = this.$root.shadowDB.Writer[this.$root.session.writer.selected].files.map(e => e.uuid).indexOf(this.$root.session.writer.file.uuid)
        //this.$root.shadowDB.Writer[this.$root.session.writer.selected].files[mykey].content=this.editor.getHTML()
        //  this.$root.shadowDB.Writer[this.$root.session.writer.selected].files[this.$root.shadowDB.Writer[this.$root.session.writer.selected].files.map(e => e.uuid).indexOf(this.$root.session.writer.file.uuid)]

        // this.$root.shadowDB.Writer[this.$root.session.writer.selected].files[mykey].wordcount = this.wordcount
        // this.changed();
      },
    });
