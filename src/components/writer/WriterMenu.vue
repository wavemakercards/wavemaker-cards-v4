<template>
  <div id="ToolHome" v-if="!$root.session.writer.selected">
    <div class="wavemaker_info_box">
      ...
      <div style="display: flex; flex-direction: column; align-items: flex-end; padding-right: 12px;">
        <SortWidget v-model="sorttype" />
      </div>
      <hr />

      <table style="width: 100%">
        <tbody>
          <tr v-for="(item, index) in listSort" :key="index" class="listRow">
            <td>
...
import SortWidget from "../universal/SortWidget.vue"; 
export default {
  name: "WriterHome",
  components: {
    Writer,
  ...
  computed: {
    listSort() {
      let list = this.toollist ? [...this.toollist] : [];
      if (this.sorttype === "name-asc") {
        return list.sort((a, b) => a.title.localeCompare(b.title));
      }
      if (this.sorttype === "name-desc") {
        return list.sort((a, b) => b.title.localeCompare(a.title));
      }
      if (this.sorttype === "lastupdated-asc") {
        return list.sort((a, b) => a.lastupdated - b.lastupdated);
      }
      if (this.sorttype === "lastupdated-desc") {
