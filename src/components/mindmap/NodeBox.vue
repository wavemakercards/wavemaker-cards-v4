<template>
  <vue-draggable-resizable
    ...
    :y="card.y"
    :resizable="false"
    @focus="onActivated(uuid)"
    @activated="onActivated(uuid)"
  ...
  >
    <div
      class="drag-handle"
      tabindex="0"
      @keydown.up.exact.prevent.stop="$root.moveNode('up')"
   ...
   <svg  viewBox="0 0 24 24">
    ...
    <button
      id="deleteButton"
      @click="deleteNode(uuid)"
      v-if="$root.session.mindmap.currentnode === uuid"
      tabindex="0"
    >
<svg  viewBox="0 0 24 24">
...
export default {
  ...
  data() {
    return {
      card: this.$root.shadowDB.Mindmap[this.$root.session.mindmap.selected]
        .content.nodes[this.uuid],
      editme: false,
      w: 200,
      h: 200,
      x: 50,
  ...
  methods: {
    textchange() {
      if (this.card.text.length > 30) {
        document.getElementById(this.uuid).style.width =
          this.card.text.length * 6.2 + "px";
      } else {
    ...
    onResize: function (x, y, width, height) {
      ...
      ].content.nodes[this.uuid].x = x;
      this.$root.shadowDB.Mindmap[
        this.$root.session.mindmap.selected
      ].content.nodes[this.uuid].y = y;
      this.$root.shadowDB.Mindmap[
        this.$root.session.mindmap.selected
      ].content.nodes[this.uuid].width = width;
      this.$root.shadowDB.Mindmap[
        this.$root.session.mindmap.selected
      ].content.nodes[this.uuid].height = height;
      this.savechange();
    },
    onDrag: function (x, y) {
      this.$root.shadowDB.Mindmap[
    ...
    deleteNode(u) {
      if (confirm(this.$root.setlang.mindmap.deletenodewarn)) {
        ...
        this.$root.shadowDB.Mindmap[
          this.$root.session.mindmap.selected
        ].content.links = this.$root.shadowDB.Mindmap[
          this.$root.session.mindmap.selected
        ].content.links.filter((v) => {
          if (v.from !== u && v.to !== u) {
            return true;
          }
        });
        // delete the node
        delete this.$root.shadowDB.Mindmap[this.$root.session.mindmap.selected]
          .content.nodes[u];
        this.savechange();
      }
    },
...
.active {
  ...
  -moz-box-shadow: 0 5px 17px -5px #000000; /*FF 3.5+*/
  -webkit-box-shadow: 0 5px 17px -5px #000000; /*Saf3-4, Chrome, iOS 4.0.2-4.2, Android 2.3+*/
  box-shadow: 0 5px 17px -5px #000000; /* FF3.5+, Opera 9+, Saf1+, Chrome, IE10 */
