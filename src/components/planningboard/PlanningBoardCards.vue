        ...
        <template v-if="MyFile">
            <div class="titleBar" @click="selectCol">
                <input type="text" class="titleBarInput" v-model="MyFile.title"
                    :placeholder="this.$root.setlang.writer.newfile" @change="updateDatabase" />
            </div>
            <div class="hscroll">
                <VueDraggableNext :list="MyFile.notes
                " tag="transition-group" group="board" class="list-group-card" :component-data="{
    tag: 'div',
    ...
    name: !drag ? 'flip-list' : null,
                    ...
                    <template v-for="(element, index) in this.MyFile.notes" :key="index">
                        <div class="list-group-card-item" tabindex="0">
                            ...
                            <button class="deleteIconButton" @click="
                                deleteNote(
                                    index,
                                    element
                                )
...
export default {
    ...
    data() {
        return {
            drag: false,
            MyFile: this.$root.useObservable(this.$root.liveQuery(async () => await this.$root.db.Files.get(this.fileId)))
        };
    },
    methods: {
        selectCol() {
            this.$root.session.writer.file = this.FileNode;
            console.log("Col Selected", this.$root.session.writer.file)
        },
        linkCard(newuuid, uuid) {
            var element = this.MyFile.notes.find(x => x.uuid === uuid);
            ...
            this.$swal(
                {
                    ...
                    if (result.isConfirmed) {
                        this.MyFile.notes.splice(index, 1);
                        this.updateDatabase();
